...
data {
  ...
  // max number of entries for given participant 
  int maxi ; 

	// y: vector of observations for y
	//	 should be scaled to mean=0,sd=1
	vector[maxi] y[n_subj] ;
	...
	// z_by_f_by_s_pad:
	int z_by_f_by_s_pad[n_subj, maxi] ;
...
parameters {
	...
	vector<lower=0>[n_z] noise_volatility ;
	
	// volatility: volatility of per-subject GPs
	vector<lower=0>[n_subj] noise_subj_volatility[n_z] ;
  ...
  // DISPLACEMENT
	...
	// volatility: volatility of population GPs
	vector<lower=0>[n_z] volatility ;

	// volatility: volatility of per-subject GPs
	vector<lower=0>[n_subj] subj_volatility[n_z] ;
...
model {
	...
	// volatility priors:
	volatility ~ normal(0, 5) ;
	noise_volatility ~ normal(0, 5) ; 
	...
	for (zi in 1:n_z){
	 	subj_volatility[zi] ~ normal(0, subj_volatility_sd[zi]) ;
	  noise_subj_volatility[zi] ~ normal(0, noise_subj_volatility_sd[zi]) ; 
	}
